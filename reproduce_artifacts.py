...
def main():
    ...
    subprocess.run([sys.executable, str(ROOT/'results/matched_betting/paired_summary.py'), '--output', str(args.output/'matched_betting_paired_summary.json')], check=True)
    ...
    subprocess.run([sys.executable, str(ROOT/'results/pooled_bias/reproduce.py'),
                    '--output', str(args.output/'pooled_bias_verification.json')], check=True)
    checks.append({'study':'pooled_bias', 'receipt':'pooled_bias_verification.json',
                   'scope':'Matched retrospective comparator formulas, stored arrays and independent archive arithmetic; no new forecast confirmation.'})
    subprocess.run([sys.executable, str(ROOT/'results/direct_target/reproduce.py'),
                    '--output', str(args.output/'direct_target')], check=True)
    checks.append({'study':'direct_target', 'receipt':'direct_target/verification.json',
                   'scope':'Exact small-sample identities and independent four-row block and archive BY replay; no new forecast confirmation.'})
    subprocess.run([sys.executable, str(ROOT/'examples/aggregate_reference_family/test_example.py')], check=True)
    checks.append({'study':'complete_reference_family_cli', 'tests':8,
                   'scope':'Exact numeric ordering, full-U and BY wiring, input contracts and counted costs; input independence is assumed.'})
    if not args.skip_figures:
        ...
        subprocess.run([sys.executable,str(ROOT/"scripts/figures/make_directional_model_figure.py"),"--input",str(ROOT/"results/directional_comparison/model_comparison.csv"),"--output-dir",str(args.output/"focused_directional"),"--focus"],check=True)
    ...
    if not args.skip_figures:
        subprocess.run([sys.executable, str(ROOT/'scripts/figures/make_validation_reader.py'), '--output', str(args.output/'reader_figures')], check=True)
    subprocess.run([sys.executable, str(ROOT/'results/reference_design/check_temporal_rank.py'), '--output', str(args.output/'cross_time_reference.json')], check=True)
    subprocess.run([sys.executable, str(ROOT/'results/reference_time_composition/check_composition.py'), '--output', str(args.output/'reference_time_composition.json')], check=True)
    subprocess.run([sys.executable, str(ROOT/'results/source_window_design/check_two_lag.py'), '--output', str(args.output/'two_lag_sources.json')], check=True)
    subprocess.run([sys.executable, str(ROOT/'results/source_window_design/check_source_design.py'), '--output', str(args.output/'source_design')], check=True)
    with (args.output/'trajectory_exact_checks.json').open('w') as receipt:
        subprocess.run([sys.executable, str(ROOT/'results/trajectory_design/contract_exact_checks.py')],
                       check=True, stdout=receipt)
    subprocess.run([sys.executable, str(ROOT/'results/trajectory_forecast/diagnostics/reproduce.py'),
                    '--output', str(args.output/'trajectory_diagnostics')], check=True)
    checks.append({'study':'trajectory_design_and_diagnostics', 'exact_checks':19,
                   'diagnostic_rows':9600, 'diagnostic_cells':32,
                   'scope':'Fixed-target design identities, all stored corrected trajectory gates, timestamp checks and matched descriptive loss intervals. No new independent confirmation.'})
    subprocess.run([sys.executable, str(ROOT/'results/shared_baseline_ranges/reproduce.py'),
                    '--output', str(args.output/'shared_baseline_ranges')], check=True)
    checks.append({'study':'shared_baseline_ranges', 'exact_weak_order_cases':2197,
                   'candidate_rows':88,
                   'scope':'Sharp structural support, fixed-draw reconstruction and unchanged model selection. No new independent confirmation.'})
    calibration_output = args.output / 'shared_baseline_calibration'
    subprocess.run([sys.executable, str(ROOT/'results/shared_baseline_calibration/study.py'),
                    '--verify', str(calibration_output)], check=True)
    subprocess.run([sys.executable, str(ROOT/'results/shared_baseline_calibration/verify_results.py'),
                    '--results', str(calibration_output),
                    '--output', str(calibration_output/'CHECK_RESULTS.json')], check=True)
    calibration_replay = json.loads((calibration_output/'REPLAY_CHECK.json').read_text())
    calibration_recount = json.loads((calibration_output/'CHECK_RESULTS.json').read_text())
    assert calibration_replay['matches'] and calibration_recount['status'] == 'PASS'
    biased_output = args.output / 'shared_baseline_biased_null'
    subprocess.run([sys.executable, str(ROOT/'results/shared_baseline_calibration/biased_null/experiment.py'),
                    '--verify', str(biased_output)], check=True)
    subprocess.run([sys.executable, str(ROOT/'results/shared_baseline_calibration/biased_null/independent_verify.py'),
                    '--results', str(biased_output),
                    '--output', str(biased_output/'INDEPENDENT_CHECK.json')], check=True)
    biased_replay = json.loads((biased_output/'REPLAY_CHECK.json').read_text())
    biased_recount = json.loads((biased_output/'INDEPENDENT_CHECK.json').read_text())
    assert biased_replay['matches'] and biased_recount['status'] == 'PASS'
    checks.append({'study':'shared_baseline_calibration',
                   'initial_protocol':{'law_budget_cells':12, 'procedure_cells':72,
                                       'independent_replications':3600, 'candidate_method_rule_rows':86400,
                                       'family_rows':21600},
                   'separate_biased_null_addendum':{'law_budget_cells':2, 'procedure_cells':12,
                                                   'independent_replications':600, 'candidate_method_rule_rows':14400,
                                                   'family_rows':3600},
                   'deterministic_replay':'PASS', 'independent_recount':'PASS',
                   'scope':'Original six finite IID laws and a separately frozen zero-target, positive-reference-bias addendum. Both protocols and all outcomes retained; no exhaustive null calibration or independent application claim.'})
    if not args.skip_figures:
        subprocess.run([sys.executable, str(ROOT/'scripts/figures/make_ieee_figures.py'),
                        '--package-root', str(ROOT), '--output', str(args.output/'paper_figures')], check=True)
        subprocess.run([sys.executable, str(ROOT/'scripts/figures/make_reference_design.py'),
                        '--output', str(args.output/'paper_figures/reference_design.pdf')], check=True)
        subprocess.run([sys.executable, str(ROOT/'scripts/figures/make_trajectory_comparison.py'),
                        '--output', str(args.output/'trajectory_figure')], check=True)
        checks.append({'study':'paper_figures', 'figures':4,
                       'scope':'IEEEtran template text and math fonts; recorded numerical coordinates retained.'})
