...
def main(package, output):
    # Retain standard TeX lookup and caller-supplied package paths.
    tex_support = package / "results/trajectory_budget_figures/tex"
    os.environ["TEXINPUTS"] = str(tex_support) + ":" + os.environ.get("TEXINPUTS", "") + ":"
    source = package / "results/shared_baseline_ranges/results/fixed_draw/certificate_all_eight.csv"
    ...
    full.set_xlim(-12.2, 1.1)
    full.set_xticks([-10, -5, 0])
    ...
    zoom.set_xlabel(r"Bound or target $(10^{-3})$" + "\nNear zero: separate scale", labelpad=5)
    ...
    for ax, axis_name in [(full, "lower_full"), (zoom, "lower_zoom")]:
        if ax is full:
            ...
            plot_points(ax, axis_name, pruned.lower, "Reallocated hybrid lower", ORANGE, "s", 3.7, z=4)
        else:
    ...
    legend_right = [Line2D([], [], ls="none", marker="o", color=BLUE, mfc="white", ms=4.0, label="Original shared"),
    ...
    fig.legend(handles=legend_right, loc="center", bbox_to_anchor=(5.30 / SIZE[0], 2.27 / SIZE[1]),
               ncol=2, frameon=False, handletextpad=.35, columnspacing=.65, borderaxespad=0)
    stem = output / NAME
    ...
    receipt = {"status": "PASS", "figure": NAME, "source": str(source.relative_to(package)),
               ...
               "near_zero_scope": "Separate labeled linear scale displays pooled lower bounds and exact census targets only. Shared bounds remain on the full scale; persistence has zero bound by declared map identity.",
...
if __name__ == "__main__":
    ...
    main(package, (args.output or package / "results/shared_baseline_figures").resolve())
