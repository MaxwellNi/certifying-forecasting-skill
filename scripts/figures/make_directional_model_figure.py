...
def main():
    ...
    parser.add_argument('--focus', action='store_true', help='Show all retail and ratings models; retain the full 82-row data export.')
    ...
    plt.rcParams.update({'font.family':'Liberation Serif','font.size':8.5,
        'mathtext.fontset':'stix','axes.titlesize':8.5,'xtick.labelsize':8.5,
    ...
    if args.focus:
        panels = [PANELS[2], PANELS[1]]
        plt.rcParams.update({'font.size':8.5, 'xtick.labelsize':8.5, 'ytick.labelsize':8.5})
        fig,axes=plt.subplots(1,2,figsize=(7.0,2.65))
        fig.subplots_adjust(left=.235,right=.995,top=.81,bottom=.18,wspace=1.50)
    else:
        panels = PANELS
            ...
            for x,color,marker,fill in [(x1,'#333333','o','white'),(x2,'#0072B2','s','#0072B2')]:
                if x is not None:
                    ...
                    ax.text(.045,y,'undefined',transform=ax.get_yaxis_transform(),
                            va='center',fontsize=8.5,color='#666666',fontstyle='italic')
                    break
    ...
    for ext in ['pdf','png','svg']:
        ...
        fig.savefig(args.output_dir/f"{'fig_public_directional_focus' if args.focus else 'fig_public_directional'}.{ext}",**kwargs)
    plt.close(fig)
    finite_pairs=sum(x['complementary'] is not None and x['directional'] is not None for x in lines)
    expected_models = 20 if args.focus else 41
    assert len(lines)==expected_models and finite_pairs==expected_models-2
    guard_count=sum(x['guarded'] for x in lines)
    receipt={'input_sha256':sha(args.input),'models':expected_models,'rows':len(paired),
             'finite_pairs':finite_pairs,'undefined_pairs':expected_models-finite_pairs,
