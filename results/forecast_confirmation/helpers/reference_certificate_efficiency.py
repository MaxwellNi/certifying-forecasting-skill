...
def family_validation_delta(family_size, q=.05, rho=.1):
    """Reserve rho of the first BY threshold for validation failure.

    Set K, q and rho before validation/evaluation. This controls p-value
    resolution only; it neither applies BY nor optimizes statistical power.
    Each family member still needs its own marginal sampling guarantee.
    """
    if isinstance(family_size, (bool, np.bool_)) or not isinstance(family_size, (int, np.integer)) or family_size < 1:
        raise ValueError('Family size must be a positive integer')
    if not np.isfinite(q) or not np.isfinite(rho) or not 0 < q < 1 or not 0 < rho < 1:
        raise ValueError('Need finite q and rho in (0,1)')
    harmonic = math.fsum(1. / k for k in range(1, int(family_size)+1))
    delta = float(rho*q/(family_size*harmonic))
    if delta <= 0:
        raise ValueError('Family validation budget underflows floating-point precision')
    return delta
...
def _raw_comparison_values(values):
    """Preserve raw integer ordering before comparisons, including beyond 2**53.

    Integer and finite real numeric arrays are accepted. Exact decimal text is
    handled by the complete-family input reader, not by this numeric helper.
    A caller that already rounded inputs to binary64 cannot recover lost order.
    """
    import math
    ...
    for index, value in np.ndenumerate(array):
        if isinstance(value, Integral):
            ...
            try:
                ...
                raise ValueError('Raw real comparison values must be finite binary64 values') from exc
            if not math.isfinite(number) or number != value:
                raise ValueError('Raw real values must be exactly representable as binary64; use exact order codes for higher precision')
            normalized[index] = number
        else:
            raise ValueError('Raw comparison values must be integers or finite real numbers')
    return normalized
...
def triple_scores(v,w,f,g):
    ...
    """
    v,w = [_raw_comparison_values(x) for x in (v,w)]
    f,g = [np.asarray(x,float) for x in (f,g)]
    ...
    if not all(np.isfinite(x).all() for x in (f,g)) or np.any((f<0)|(f>1)|(g<0)|(g>1)):
...
def one_sided_certificate(mean,variance_scores,effective,f,g,bias_upper,delta=.0001,alpha=.05,kind='variance'):
    ...
    if kind=='range':
        radius=R*np.sqrt((-np.log(alpha-delta))/(2*effective));p=min(1.,delta+np.exp(-2*effective*(margin/R)**2));s2=None
        ...
        s2=float(h.var(ddof=1));x=(np.log(2.)-np.log(alpha-delta));A=np.sqrt(2*s2/effective)
...
def planned_triples(min_effect,bias_width,f,g,alpha=.05,delta=.0001,beta=.2):
    ...
    amount=(hi-lo)**2*(np.sqrt((-np.log(alpha-delta)))+np.sqrt((-np.log(beta))))**2/(2*(min_effect-bias_width)**2)
