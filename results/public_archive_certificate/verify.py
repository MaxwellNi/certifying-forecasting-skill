...
def runtime_source_overrides():
    """Verify explicit current-source replacements without altering old receipts.

    A fresh source reproduction can use its own current protocol without this
    file. A changed historical source requires an exact old-to-current link in
    the complete artifact, and every listed current source is hash-checked.
    """
    root = HERE.parents[1]
    ...
    assert document['format'] == 'explicit-source-replacements-1'
    entries = document['files']
    assert isinstance(entries, dict) and entries
    for name, entry in entries.items():
        relative = Path(name)
        assert not relative.is_absolute() and '..' not in relative.parts
        assert len(entry['historical_sha256']) == len(entry['current_sha256']) == 64
        assert sha(root / relative) == entry['current_sha256'], ('current replacement hash', name)
        assert (root / relative).stat().st_size == entry['current_bytes'], ('current replacement size', name)
    regression = document['regression_check']
    assert regression['passed'] is True and regression['test_methods'] > 0
    assert sha(root / regression['test_file']) == regression['test_sha256']
    return root, entries, sha(path)


def check_recorded_source(path, historical_digest, root, replacements):
    current = sha(path)
    if current == historical_digest:
        return {'path': str(path.relative_to(HERE)), 'mode': 'unchanged'}
    name = str(path.relative_to(root))
    assert name in replacements, ('unrecorded source replacement', name)
    entry = replacements[name]
    assert entry['historical_sha256'] == historical_digest, ('historical source link', name)
    assert entry['current_sha256'] == current, ('current source link', name)
    return {'path': name, 'mode': 'explicit_replacement',
            'historical_sha256': historical_digest, 'current_sha256': current}


def hashes():
    return {str(p.relative_to(HERE)): sha(p) for p in HERE.rglob('*')
            if p.is_file() and '__pycache__' not in p.relative_to(HERE).parts
...
def main():
    ...
    source_root, source_replacements, replacement_digest = runtime_source_overrides()
    ...
    for name, entry in p['helper_sources'].items():
        historical_source_checks.append(check_recorded_source(
            HERE / 'helpers' / name, entry['sha256'], source_root, source_replacements))
    ...
    if replacement_digest is not None:
        assert sha(source_root / 'SOURCE_OVERRIDES.json') == replacement_digest
    print(json.dumps(dict(
        passed=True, files_unchanged=True, release_manifest_checked=manifest_checked,
        historical_helper_source_checks=historical_source_checks,
        current_source_replacements_verified=len(source_replacements),
        source_replacement_record_sha256=replacement_digest,
